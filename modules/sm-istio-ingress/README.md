...
| <a name="input_ingress_deployment_name"></a> [ingress\_deployment\_name](#input\_ingress\_deployment\_name) | Optional override for the ingress Deployment name. If null or empty, the default name is used. | `string` | `null` | no |
...
| <a name="input_ingress_networkpolicy_enabled"></a> [ingress\_networkpolicy\_enabled](#input\_ingress\_networkpolicy\_enabled) | Enable creation of the ingress NetworkPolicy (requires istioselectors to be non-empty). | `bool` | `false` | no |
