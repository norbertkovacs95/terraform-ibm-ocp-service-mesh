...
| <a name="input_mesh_config_extension_providers"></a> [mesh\_config\_extension\_providers](#input\_mesh\_config\_extension\_providers) | List of mesh-wide extension providers to place under spec.values.meshConfig.extensionProviders. | `list(any)` | `null` | no |
...
| <a name="input_pilot_env"></a> [pilot\_env](#input\_pilot\_env) | Optional map of environment variables to set on Istio Pilot (key=value). | `map(string)` | `{}` | no |
